...
function multiplyByIndex(arr) {
  ...
  let newArray = [];
  for (const element of arr) {
    newArray[newArray.length] = element * newArray.length;
  }
  return newArray;
}
...
function replaceNumbers(arr) {
  ...
  for (const element of arr) {
    if (element > 10) {
      newArray[newArray.length] = "Greater than 10";
    } else if (element <= 10) {
      newArray[newArray.length] = "Less than or equal to 10";
    }
  }
  return newArray;
}
